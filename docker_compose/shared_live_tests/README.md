# Shared live security test (fa-ui hardened reference)

This example runs the full `security-tests-m8` live security suite against the `fa-ui-m8` `hardened_ui_m8` Docker Compose stack.
...
> and `fastapi-m8`-based consumers**. In this repo the stack bundles the
> `fa-auth-m8` auth service and the `media-service-m8` consumer behind Traefik.
> Only the configuration in `.env` changes — see
> [Adapting To Another Stack](#adapting-to-another-stack). `hardened_ui_m8` is
> the reference target documented below.

- Local compose stack path: `/workspace/fa-ui-m8/docker_compose/hardened_ui_m8`
- Local live-test example folder: `/workspace/fa-ui-m8/docker_compose/shared_live_tests`
...
- auth service (issuer): `http://localhost:9000/user` (`fa-auth-m8`, `API_PREFIX=/user`)
- downstream media service: `http://localhost:9000/media` (`media-service-m8`, `API_PREFIX=/media`)
- public HTTPS entrypoint: `https://localhost:4430` (Traefik, self-signed local cert)
- stack root and JWT keys: `/workspace/fa-ui-m8/docker_compose/hardened_ui_m8`
...
CLI mode is recommended for normal users and excludes destructive tests by default. This local pytest example is for custom tests, extra marker selection, and local suite extension.
...
└── tests/live/
    ...
    └── test_media_exposure_matrix.py
```

`test_media_exposure_matrix.py` is the fa-ui-specific **`/media` half of the
item 5.2 live exposure matrix**. The reusable package suites
(`Private/Metrics/HealthAPISuite`, wired in via `test_full_security.py`) cover
the `/user` issuer surface; this local test adds the downstream
`media-service-m8` (`/media`) surface — `/media/metrics` and deep
`/media/health/` proxy-excluded (404), `/media/internal/*` app-gated — and is
parameterized by deployment topology with `EXPOSURE_TOPOLOGY={case_a|case_b}`
(default `case_b`, the shipped hardened stack where `/media` is public).
...
```bash
cd /workspace/fa-ui-m8/docker_compose/hardened_ui_m8
cp .env.example .env
...
Install (or update) `security-tests-m8`:
...
```bash
cd /workspace/fa-ui-m8/docker_compose/shared_live_tests
cp env.example .env
pytest
```
...
Nothing in this folder is specific to `hardened_ui_m8` beyond the values in
...
2. **Service URL(s)** — `LIVE_TEST_SVC_BASE` / `LIVE_TEST_SVC_BASES` with
