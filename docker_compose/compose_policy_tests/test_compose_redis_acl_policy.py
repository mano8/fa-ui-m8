...
Both UI stacks bundle the full auth + media platform, so each ships two Redis
services that must run a scoped ACL bootstrap instead of the old open
``appuser ~* +@all``:
...
- ``redis_cache`` — the bundled **auth** service's Redis. The auth app
  ...
  (identical to fa-auth-m8 / media-service-m8).
- ``media_redis_cache`` — the bundled **media** service's Redis (rate limits +
  the ``arq`` queue). The media app authenticates as a scoped ``media`` user
  restricted to the ``media:*`` namespace and the ``arq:*`` queue keys.
...
# Both committed UI stacks ship the two redis services with an ACL bootstrap.
_STACKS = ["dev_ui_m8", "hardened_ui_m8"]
...
# Key prefixes each bundled service writes; every one must be covered by an ACL
# ``~prefix*`` pattern (audited from fa-auth-m8 / media-service-m8).
_KEY_PREFIXES = {
    "auth": [
        "oauth_session:",
        "auth_code:",
...
@pytest.mark.parametrize("stack", _STACKS)
@pytest.mark.parametrize("service,user,_env,_var", _SERVICES)
def test_acl_covers_every_audited_key_prefix(
    stack: str, service: str, user: str, _env: str, _var: str
) -> None:
    patterns = _acl_key_patterns(_setuser_line(_redis_command(stack, service), user))
    globs = [p[:-1] if p.endswith("*") else p for p in patterns]
    for prefix in _KEY_PREFIXES[user]:
        assert any(prefix.startswith(g) for g in globs), (
            f"{stack}/{service}: key prefix {prefix!r} not covered by ACL {patterns}"
        )
