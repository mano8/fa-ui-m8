...
| --- | --- | --- |
| `.env.example` → `.env` | Compose itself (`${VAR}` interpolation) and the engine init scripts | `DB_PASSWORD`, `AUTH_DB_USER`, `AUTH_DB_PASSWORD`, `MEDIA_DB_USER`, `MEDIA_DB_PASSWORD`, `REDIS_PASSWORD`, `MEDIA_REDIS_PASSWORD`, `S3_ROOT_USER`, `S3_ROOT_PASSWORD` |
| `.env.production.example` → `.env.production` | Compose itself (`${VAR}` interpolation) and the engine init scripts | `DB_PASSWORD`, `AUTH_DB_USER`, `AUTH_DB_PASSWORD`, `MEDIA_DB_USER`, `MEDIA_DB_PASSWORD`, `REDIS_PASSWORD`, `MEDIA_REDIS_PASSWORD`, `S3_ROOT_USER`, `S3_ROOT_PASSWORD` |
...
| `media.env.example` → `media.env` | `media_service`, `storage-config`, `storage-init` | `DB_USER`, `DB_PASSWORD`, `MEDIA_REDIS_PASSWORD`, `MEDIA_INTERNAL_SERVICE_TOKEN`, `MEDIA_SHARE_SIGNING_SECRET`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`, `REFRESH_SECRET_KEY`, `PRIVATE_API_SECRET`, `EVENT_SIGNING_KEY` |
| `media.env.production.example` → `media.env.production` | `media_service`, `storage-config`, `storage-init` (via `docker-compose.production.yml`) | `DB_USER`, `DB_PASSWORD`, `MEDIA_REDIS_PASSWORD`, `MEDIA_INTERNAL_SERVICE_TOKEN`, `MEDIA_SHARE_SIGNING_SECRET`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`, `REFRESH_SECRET_KEY`, `PRIVATE_API_SECRET`, `EVENT_SIGNING_KEY` |
